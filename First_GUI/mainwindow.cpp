...

{
    ui->lbl_result->setText(QString::fromStdString(""));
     ...
     if(game.isValid(src) && game.isValid(dest)){
         ...
         int x1 = src[0] - 97;
         int y1  = src[1]-48;
if(!game.isReachableB())
{
 ui->lbl_result->setText(QString::fromStdString("Invalid color"));
}
else
     {
...
void MainWindow::on_btn_Calculate_clicked()
{
    ...
    }else if(game.src->safe==false){
        ui->lbl_result->setText(QString::fromStdString("The starting position is not safe"));
    }
    else if(button1){
    ...
    {
        game.addPathB((game.dest),0);
        game.choosePathB();

        if(!game.pathB.empty()){
            setInitial1(game.pathB.front()->pos);
            game.pathB.pop();}
        if(!game.pathB.empty()){
        setInitial2(game.pathB.front()->pos);
        game.pathB.pop();}
        if(!game.pathB.empty()){
        setInitial3(game.pathB.front()->pos);
        game.pathB.pop();}
    }
